...
1. 在多个轻量级开源指令模型上，`Self-Consistency` 是否相比单条 `CoT` 带来更稳定或更高的准确率。
...
本项目任务 A 复现的是 Wang et al. 主论文中的 CoT 与 Self-Consistency 方法机制，而不是固定复现某一组特定大模型 checkpoint。考虑到课程集群 GPU 时间和显存资源有限，正式实验选择当前硬件条件下可在 24 小时量级内完成完整矩阵的轻量级开源指令模型：
...
项目早期曾尝试 `mistral_7b_instruct_v0_3`、`olmo3_7b_instruct` 和 `llama2_7b_hf` 等 7B 级模型，但在当前计算资源下完整运行 `3 模型 × 3 数据集 × 3 方法`，尤其是 `num_paths = 5` 的 Self-Consistency，预计需要数天到一周。为保证课程项目可按时完成并为任务 B/C 提供完整 baseline，正式口径改为上述轻量模型。
...
- 对每个数据集固定随机采样 `100` 条样本
- 随机种子固定为 `42`
- `GSM8K`、`CSQA`、`MMLU` 分别生成一份固定 sampled subset
- 任务 A、任务 B、任务 C 均使用同一批 sampled subset

生成长度设置为 `zero_shot/few_shot max_new_tokens = 256`，`self_consistency max_new_tokens = 384`；SC 路径数设为 `num_paths = 3`，用于在有限算力下保留多路径采样与投票的核心机制。
...
考虑到任务 A 需要在两天内完成 baseline 结果并交接给后续任务，本项目将任务 A 的正式执行口径固定为 `lightweight-100`：

- 任务 A 保留完整实验矩阵，即 `3 模型 × 3 数据集 × 3 方法 = 27` 个配置
- 每个配置使用同一随机种子 `42` 采样 `100` 条样本
- `Self-Consistency` 采用小规模多路径设置 `num_paths = 3`
- 作业按 `model × mode` 拆分批次运行
- 执行顺序为先全部 `zero_shot`，再全部 `few_shot`，最后全部 `self_consistency`

该设置优先保证 baseline 结构完整、结果可复现、后续任务可直接复用，同时将实验规模控制在课程项目周期内。
...
- 3 个模型：Qwen2.5-0.5B-Instruct、Qwen2.5-1.5B-Instruct、TinyLlama-1.1B-Chat
...
- 27 个 baseline 配置均完成 lightweight-100 评测
- 结果文件可复现、可追踪
- 能明确回答 SC 是否优于单条 CoT
...
- 1 到 2 个代表模型，例如 Qwen2.5-1.5B-Instruct 和 TinyLlama-1.1B-Chat
- 3 个数据集均可覆盖
...
- 1 个主模型，例如 Qwen2.5-1.5B-Instruct
...
- 使用固定随机种子的 lightweight-100 评测口径
- 所有任务共享同一批样本
